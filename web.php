...
Route::get('fix_invoices_4', function () {
    ...
    foreach ($transactions1 as $transaction) {
        ...
        foreach ($sellLines as  $sellLine) {
            ...
            $tax = $sellLine->unit_price_inc_tax - ($sellLine->unit_price_inc_tax * 100 / 115);
            $sellLine->update([
                'tax_id' => 50,
            ...
            $total_tax += ($tax * $sellLine->quantity);
        }
        $transaction->update([
            'tax_id' =>  50,
            'total_before_tax' => $transaction->final_total -  2 * $total_tax,
    ...
    foreach ($transactions2 as $transaction) {
        ...
        foreach ($sellLines as  $sellLine) {
            ...
            $tax = ($sellLine->unit_price_inc_tax - ($sellLine->unit_price_inc_tax * 15 / 100)) * 100 / 200;
            $sellLine->update([
                'tax_id' => 81,
                'unit_price' => $tax,
                'item_tax' => $tax,
        ...
        $total_tax = $total_tax * 15 / 100;
        $transaction->update([
            'tax_id' =>  50,
            'total_before_tax' => $transaction->final_total - $total_tax,
            'tax_amount' => $total_tax,
        ]);
    }
    ...
    foreach ($transactions3 as $transaction) {
        ...
        foreach ($sellLines as  $sellLine) {
            ...
            if ($sellLine->product->tax != null && $sellLine->product->tax == 50) {
                ...
                $not_tobaco += ($tax * $sellLine->quantity);
